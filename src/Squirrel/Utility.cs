...
namespace Squirrel
{
    static class Utility
    {
        ...
        public static Uri AddQueryParamsToUri(Uri uri, IEnumerable<KeyValuePair<string, string>> newQuery)
        {
            ...
            var existingQuery = uri.Query.TrimStart('?').Split(new[] { '&' }, StringSplitOptions.RemoveEmptyEntries);
            // Replace System.Web.HttpUtility with a manual approach to parse query strings
            var query = new NameValueCollection();

            foreach (var pair in existingQuery)
            {
